...
@Component
public class ConsumerStatusJob implements SimpleJob {
    @Autowired
    private ConsumerCenterPool consumerCenterPool;
    @Override
    public void execute(ShardingContext shardingContext) {
        Map<String, AtomicBoolean> status= consumerCenterPool.getMqConsumerStatus();
        ...
        while (iterator.hasNext()){
            ...
            if(StringUtils.isEmpty(result.toString())){
                result.append("消费者实例列表运行状态为").append(":");
