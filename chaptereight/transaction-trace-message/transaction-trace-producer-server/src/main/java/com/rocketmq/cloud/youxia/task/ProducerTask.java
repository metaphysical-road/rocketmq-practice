...
import com.rocketmq.cloud.youxia.config.TransactionTraceProducerConfig;
...
public class ProducerTask {
    ...
    @Autowired
    private TransactionTraceProducerConfig transactionTraceProducerConfig;
    ...
    public void producerMessage() throws MQClientException,InterruptedException, UnsupportedEncodingException {
        if (flag.compareAndSet(false, true)) {
            //定义一个生产者对象mqProducer
            TransactionMQProducer mqProducer = new TransactionMQProducer(transactionTraceProducerConfig.getNameSpace(),
                    transactionTraceProducerConfig.getProducerGroup(), null, true,
                    transactionTraceProducerConfig.getTraceTopic());
            //设置Name Server的IP地址
            mqProducer.setNamesrvAddr(transactionTraceProducerConfig.getNameAddress());
            //启动生产者
            mqProducer.start();
            //定义消息主题名称
            String topic = transactionTraceProducerConfig.getTopic();
