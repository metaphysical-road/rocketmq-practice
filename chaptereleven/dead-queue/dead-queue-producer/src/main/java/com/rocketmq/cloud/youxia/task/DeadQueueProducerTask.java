...
import com.rocketmq.cloud.youxia.config.DeadQueueProducerConfig;
...
@Component
@EnableScheduling
public class DeadQueueProducerTask {
    ...
    @Autowired
    private DeadQueueProducerConfig deadQueueProducerConfig;

    @Scheduled(fixedRate = 1000)
    public void producerMessage() {
        String isSingleInstance = deadQueueProducerConfig.getIsSingleInstance();
        if (isSingleInstance.equals("false")) {
            String instanceName = deadQueueProducerConfig.getInstanceName();
            ...
            if (instanceNameList.length > 0) {
                for (String s : instanceNameList) {
                    if (!defaultMQProducerMap.containsKey(s)) {
                        DefaultMQProducer defaultMQProducer = new DefaultMQProducer(deadQueueProducerConfig.getProducerGroup());
                        defaultMQProducer.setNamesrvAddr(deadQueueProducerConfig.getNamesrvAddr());
                        defaultMQProducer.setInstanceName(s);
                        defaultMQProducer.setClientIP(deadQueueProducerConfig.getClientIp());
                        try {
            ...
            String singleInstanceName = deadQueueProducerConfig.getSingleInstanceName();
            String multiProducerGroup = deadQueueProducerConfig.getMultiProducerGroup();
        ...
        if (CollectionUtils.isNotEmpty(defaultMQProducerList)) {
            ...
            if(deadQueueProducerConfig.getSendLatencyFaultEnable().equals("true")){
                random.setSendLatencyFaultEnable(true);
            }
            try {
                //生产消息
                String topic = deadQueueProducerConfig.getTopic();
                //构造消息体
                Message msg = new Message(topic,
                        ("this is a test message" + RandomUtils.nextLong(1, 20000000)).
                                getBytes(RemotingHelper.DEFAULT_CHARSET));
                //开启延迟
                msg.putUserProperty("delay",deadQueueProducerConfig.getDelay());
                //开启延迟时间
                msg.putUserProperty("delayTime",deadQueueProducerConfig.getDelayTime());
