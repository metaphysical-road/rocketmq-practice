...
import com.rocketmq.cloud.youxia.config.HaRetryProducerConfig;
...
@Component
@EnableScheduling
public class HaRetryProducerTask {
    ...
    @Autowired
    private HaRetryProducerConfig haRetryProducerConfig;

    @Scheduled(fixedRate = 1000)
    public void producerMessage() {
        String isSingleInstance = haRetryProducerConfig.getIsSingleInstance();
        if (isSingleInstance.equals("false")) {
            String instanceName = haRetryProducerConfig.getInstanceName();
            ...
            if (instanceNameList.length > 0) {
                for (String s : instanceNameList) {
                    if (!defaultMQProducerMap.containsKey(s)) {
                        DefaultMQProducer defaultMQProducer = new DefaultMQProducer(haRetryProducerConfig.getProducerGroup());
                        defaultMQProducer.setNamesrvAddr(haRetryProducerConfig.getNamesrvAddr());
                        defaultMQProducer.setInstanceName(s);
                        defaultMQProducer.setClientIP(haRetryProducerConfig.getClientIp());
                        try {
            ...
            String singleInstanceName = haRetryProducerConfig.getSingleInstanceName();
            String multiProducerGroup = haRetryProducerConfig.getMultiProducerGroup();
        ...
        if (CollectionUtils.isNotEmpty(defaultMQProducerList)) {
            ...
            if(haRetryProducerConfig.getSendLatencyFaultEnable().equals("true")){
                random.setSendLatencyFaultEnable(true);
            }
            try {
                //生产消息
                String topic = haRetryProducerConfig.getTopic();
                //构造消息体
                Message msg = new Message(topic,
                        ("this is a test message" + RandomUtils.nextLong(1, 20000000)).
                                getBytes(RemotingHelper.DEFAULT_CHARSET));
                //开启延迟
                msg.putUserProperty("delay",haRetryProducerConfig.getDelay());
                //开启延迟时间
                msg.putUserProperty("delayTime",haRetryProducerConfig.getDelayTime());
